            ...
            <div class="mb-6 sm:mb-8">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p class="text-sm sm:text-base text-gray-600">Kelola produk yang telah dihapus</p>
                    </div>
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                ...
                <div class="bg-red-50 px-6 py-4 border-b border-gray-100">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h2 class="text-lg font-bold text-gray-900">Produk Terhapus</h2>
                            <p class="text-xs text-gray-600">{{ $products->total() }} produk di trash</p>
                        </div>
                    </div>
                </div>
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center space-x-2">
                                            <form action="{{ route('operator.products.restore', $product->id) }}" method="POST" class="inline">
                                                @csrf
                                            ...
                                            <form action="{{ route('operator.products.force-delete', $product->id) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-sm text-gray-500">Tidak ada produk yang dihapus</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
...
async function handleRestoreProduct(button, productName) {
    const confirmed = await Swal.fire({
        ...
        html: `Apakah Anda yakin ingin memulihkan produk "<strong>${productName}</strong>"?`,
        icon: 'question',
...
async function handleForceDeleteProduct(button, productName) {
    const confirmed = await Swal.fire({
        ...
        html: `Apakah Anda yakin ingin menghapus permanen produk "<strong>${productName}</strong>"?<br><br><span class="text-red-600 font-semibold">Tindakan ini tidak dapat dibatalkan!</span>`,
        icon: 'warning',
        showCancelButton: true,
    ...
    if (confirmed.isConfirmed) {
        const form = button.closest('form');
        loadingOverlay.show('Menghapus produk permanen...');
        form.submit();
    }
}
</script>
